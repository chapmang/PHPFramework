	...
	class Log {
		...
		public static function exception($e) {
			...
			if (Configuration::get('error.logPath')) {
				$logFile = Configuration::get('error.logPath');
			} else {
				// Default framework log location 
				$logFile = path('storage') . "logs/errors/" . date("Y-m-d") . ".log";
			}

			// Write the entry to the log file
			$message = $e->getMessage() . ' in ' . $e->getFile() . ' on line ' . $e->getLine();
			
			static::write('error', $logFile, $message);
		}
		 ...
		 * @return void           	
		 */
		public static function write($type, $logFile, $message) {
